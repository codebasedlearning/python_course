...
"""
This is the script or module description, see below.
"""
...
Doc-Strings
  - Many rules about formatting and naming can be found in 
    ...
    should be mentioned. 
  - The module description at the beginning may be presented in a slightly different style 
    to other block comments. 
    This description would be displayed in a tooltip in an IDE where you 
    import the script, i.e. on the 'import' command. You can try this for example 
    with the description of 'platform'. 
"""


# first look at a typical function

def scale_if_positive(n):
    """ Scales n if positive, otherwise returns 0 """               # doc string, hold mouse on 'scale_if_positive'

    result = 0                                                      # spaces/indent and a local variable
    # result: int = 0                                               # type hints (not today)
    if n > 0:                                                       # code blocks, indention level
        case = 1                                                    # scope of 'case'?
        result = n*3
    else:
        case = 2
    print(f" a|   {n=}, {result=} | {case=}")                       # ' n=' self-documenting expression
    return result
...
'Pythonic way'
  - Defining the main functionality of the program in its own functions, which are then 
    called, is one example of best practice, as they say here, the so-called 
    'Pythonic way'. This means, among other things: Do something right from 
...
Code (from Style Guide for Python Code, PEP 8)
  ...
  - variable names follow the same convention as function names
      ...
      * no standard file header, use VCS instead (see example below)
      * use type hints if needed (later)
      * define something like 'main', avoid variable shadowing

(B) File information.
This discussion always comes up when it comes to a (the?) 'common standard header' format of a Python script. 
Quoted from Jonathan Hartley, stackoverflow, could not put it any better:
    I think all of this metadata after the imports is a bad idea. The parts of this metadata that apply to a single file 
    (e.g. author, date) are already tracked by source control. Putting an erroneous & out of date copy of the same info 
    in the file itself seems wrong to me. 
    The parts that apply to the whole project (eg licence, versioning) seem better located at a project level, in a file 
    of their own, rather than in every source code file.
That's why you don't find this information in the snippets.



(H) Variable shadowing.
In this code (in comments) we call 'twice' directly and assign the result to 'n'. 
However, now 'n' exists globally and this leads to a warning that the parameter 'n' in 'twice' covers=shadows this 
(global) variable. If we move the 'n=...' call into its own function, e.g. 'main', this will not happen.
