...
_task_regex = re.compile(r"Task-(\d+)")
...
def _current_task_name() -> str:
    try:
        task = asyncio.current_task()      # RuntimeError if no loop on THIS thread
    except RuntimeError:
        return "-"                         # worker thread / sync code: no task here
...
def _timing_info(*, incl_proc=False, incl_task=False):
    ...
    if incl_proc:
        ...
        pinfo = ""
    if incl_task:
        ttinfo = f"{_task_name_translation(_current_task_name()):>5}:"
    else:
        ttinfo = ""
    return f"[{pinfo}{ttinfo}{tinfo}{diff: >6.3f}]"
...
def reset_timing_now():
    """ Reset the relative-time clock immediately """
    _reset_timing_info()
...
def ttprint(label, *args, **kwargs):
    print(f"{_timing_info(incl_task=True)} {label}", *args, **kwargs)
