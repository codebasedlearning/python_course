...
> Für einen Zeitschriftenverlag soll ein Programm zur Erstellung von Kreuzworträtseln realisiert
werden. 

Sie bekommen eine Menge von Wörtern vorgegeben und sollen diese dann horizontal und 
vertikal so anordnen, dass sie miteinander verbunden sind und möglichst **wenig Platz** einnehmen
(minimales Kompaktheitsmass).
Die übrigen Felder werden mit zufälligen Buchstaben aufgefüllt. Die oder der Leser:in
der Zeitschrift muss dann später diese Wörter in dem Buchstabensalat finden. Das Finden ist aber
hier 
...
Die erste Anordnung nimmt 5 · 6 = 30 Felder ein (Kompaktheitsmass) und ist damit besser als die
zweite 
mit 5 · 8 = 40 Feldern.
...
Die nachfolgende Anordnung ist ebenfalls erlaubt, denn es müssen keine 'Leerzeilen' zwischen den
Wörtern
...
Nicht erlaubt hingegen ist die folgende Variante, da es keine Verbindungen zwischen den Wörtern
gibt:
...
Die Ausgabe auf den Bildschirm soll zunächst die nicht versteckte Anordnung und danach, optional,
die
...
- Haben die Wörter keine gemeinsamen Buchstaben, z.B. bei SACHE und WORT, dann gibt es auch keine
  Lösung.
...
enthaltene Wort steht als verbindendes Wort nicht mehr zur Verfügung. Daher sollen die Wörter nicht
'eingebettet'
werden.
...
- In der Implementierung für den Verlag wird zwar gefordert, die Buchstaben noch zu verstecken. In
  Ihrer
...
Implementieren Sie unter Einhaltung der beschriebenen Randbedingungen ein Programm zur Erstellung
von Kreuzworträtseln mit möglichst geringer Kompaktheit.
