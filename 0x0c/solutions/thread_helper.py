...
threadpool_regex = re.compile(r"ThreadPoolExecutor-(\d+)_(\d+)")
async_regex = re.compile(r"asyncio_(\d+)")
...
def name_translation(name:str):
    ...
    elif match:=async_regex.search(name):
...
def dt(reset=False):
    if not hasattr(dt, 't0') or reset:
        ...
        dt.t0 = time.monotonic()
    # diff = time.perf_counter() - dt.t0
    diff = time.monotonic() - dt.t0
