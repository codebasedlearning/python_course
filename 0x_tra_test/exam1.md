...
Frage: Welche Funktion besitzt ein führender Unterstrich?
Schreiben Sie Ihre Antwort in den Doc-String der Initialisierungsfunktion, 
Stichwort genügt.
...
In `count_valid_passwords` nutzen Sie `entries` um die Anzahl gültiger Passwörter 
...
```
    ...
    print([...])
```
Vervollständigen Sie das `print`-Kommando in `sensors` und nutzen Sie 
...
c) [4P] Kopieren Sie die Generatorfunktion `sensor_a` zu `sensor_b` 
...
lautet hier (Reihenfolge unerheblich):
```
    {'c': [('c', 25)], 'a': [('a', 23), ('a', 28)]}
```
...
f) [4P] Beim Verwenden von `sensor_a` mit mehreren Threads gibt es 
...
(entweder die Sensorwerte oder das ganze Tuple).
